...
use crate::clientside::xdg_activation::ActivationData;
...
impl<C: XConnection> ServerState<C> {
    ...
    pub fn new_window(
        ...
        let activation_token = if let Some(pid) = pid.filter(|pid| self.pids.insert(*pid)) {
    ...
    fn handle_activations(&mut self) {
        ...
        for (window, token) in globals.pending_activations.drain(..) {
